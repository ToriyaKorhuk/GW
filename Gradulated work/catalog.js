  ...
  /*   <div class="item">
    ...
    createProducts(){
        ...
        for(let i = 0; i < this.catalogProducts.length; i++){
            // if(
            //     this.catalogProducts[i].price<3
            // ){
            ...
            //pthis заменяем на обьект, который ему принадлежит
...
let allProducts = new AllProducts('.container_products', catalogProduct, '.catalog_counter');
 ...
 var ger = document.getElementById('ser');
function getFilterProducts(){
    let result = [];
   if( location.pathname.substring(location.pathname.lastIndexOf("/") + 1) === "index.html"){
    for(let i=0; i < catalogProduct.length; i++){
        if(catalogProduct[i].name === 'Серьги'){
          onclick(ger).innerHTML = result.push(catalogProduct[i]);
        }
    }
}
}
